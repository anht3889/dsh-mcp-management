 ...
 * @module @deepseek-ai/dsh-mcp-mgmt-manager/runtime
 */

import { mkdir } from 'node:fs/promises'
import { dirname } from 'node:path'
import type { Context } from '@deepseek-ai/cordis'
import { McpRuntime } from '@deepseek-ai/dsh-mcp-mgmt-mcp'
import type {
  McpConnectionStatus,
  McpLogEntry,
  McpServerId,
...
export interface McpManagerRuntimeOptions extends McpManagerRuntimeConfig {
  ...
  /** OAuth callback URI; Task 9 replaces the loopback placeholder with the live web-server origin. */
 ...
 */
export class McpManagerRuntime extends McpRuntime {
  ...
  private constructor(
    ...
    this.oauth = createOAuthController({
      ...
      redirectUri: options.oauthRedirectUri ?? `http://127.0.0.1${OAUTH_REDIRECT_PATH}`,
    })
  }
   ...
   */
  override get(id: McpServerId): McpServerRecord | undefined {
   ...
   */
  override async upsert(record: McpServerRecord): Promise<McpServerRecord> {
   ...
   * @param id - server identifier.
   */
  override async remove(id: McpServerId): Promise<void> {
   ...
   */
  override async setEnabled(id: McpServerId, enabled: boolean): Promise<void> {
   ...
   * @param id - server identifier.
   */
  override async connect(id: McpServerId): Promise<void> {
    const record = this.requireRecord(id)
    await this.disconnect(id)
   ...
   * @param id - server identifier.
   */
  override async disconnect(id: McpServerId): Promise<void> {
   ...
   */
  override getStatus(id: McpServerId): McpConnectionStatus {
   ...
   */
  override getLogs(id: McpServerId, after?: number): { next: number; entries: McpLogEntry[] } {
   ...
   */
  override async startOAuth(id: McpServerId): Promise<{ authorizeUrl: string }> {
    this.requireRecord(id)
   ...
   * @param id - OAuth-configured server identifier.
   */
  override async clearOAuth(id: McpServerId): Promise<void> {
    this.requireRecord(id)
   ...
   */
  override async setSecrets(id: McpServerId, secrets: Record<string, string>): Promise<void> {
    this.requireRecord(id)
